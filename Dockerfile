...
# Set Node.js heap size to prevent out of memory errors
...
# Build with increased memory allocation and in stages to reduce memory usage
RUN \
  yarn build:deps \
  && yarn build:app \
  && rm -rf node_modules \
  ...
  && yarn install --frozen-lockfile --production=true --ignore-optional --network-timeout 1000000
...
# Set Node.js heap size for runtime
